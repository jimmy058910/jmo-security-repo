...
def test_print_header(capsys):
    ...
    from wizard import _print_header
...
def test_print_step(capsys):
    """Test step printing."""
    from wizard import _print_step
...
def test_prompt_choice_with_retry(mock_input):
    ...
    from wizard import _prompt_choice
...
def test_prompt_text_custom(mock_input):
    ...
    from wizard import _prompt_text
...
def test_prompt_yes_no_retry(mock_input):
    ...
    from wizard import _prompt_yes_no
...
def test_detect_docker():
    ...
    from wizard import _detect_docker
...
def test_check_docker_running():
    ...
    from wizard import _check_docker_running
...
def test_detect_repos_in_dir(tmp_path):
    ...
    from wizard import _detect_repos_in_dir
...
def test_validate_path_existing(tmp_path):
    ...
    from wizard import _validate_path
...
@patch("wizard._detect_docker", return_value=True)
@patch("wizard._check_docker_running", return_value=True)
...
@patch("wizard._prompt_choice", return_value="balanced")
def test_select_profile(mock_choice):
    """Test profile selection."""
    from wizard import select_profile
...
@patch("wizard._prompt_choice", return_value="tsv")
@patch("wizard._prompt_text", side_effect=["repos.tsv", "repos-dest"])
def test_select_target_tsv_mode(mock_text, mock_choice):
    """Test target selection for TSV mode."""
    from wizard import select_target

    mode, path, tsv_path, tsv_dest = select_target()
    assert mode == "tsv"
    assert path == ""
    assert tsv_path == "repos.tsv"
    assert tsv_dest == "repos-dest"


@patch("wizard._prompt_choice", return_value="targets")
@patch("wizard._prompt_text", return_value="/path/to/targets.txt")
@patch("wizard._validate_path", return_value=Path("/path/to/targets.txt"))
def test_select_target_targets_mode(mock_validate, mock_text, mock_choice):
    """Test target selection for targets file mode."""
    from wizard import select_target

    mode, path, tsv_path, tsv_dest = select_target()
    assert mode == "targets"
    assert path == "/path/to/targets.txt"
    assert tsv_path == ""
    assert tsv_dest == ""


@patch("wizard._prompt_choice", return_value="repos-dir")
@patch("wizard._prompt_text", side_effect=["", "/valid/path"])
@patch("wizard._validate_path", return_value=Path("/valid/path"))
@patch("wizard._detect_repos_in_dir", return_value=[])
@patch("wizard._prompt_yes_no", return_value=True)
def test_select_target_empty_path_retry(
    mock_yes_no, mock_detect, mock_validate, mock_text, mock_choice
):
    """Test target selection with empty path retry."""
    from wizard import select_target

    mode, path, _, _ = select_target()
    assert mode == "repos-dir"
    assert mock_text.call_count == 2  # First empty, second valid


@patch("wizard._prompt_choice", return_value="repos-dir")
@patch("wizard._prompt_text", return_value="/valid/path")
@patch("wizard._validate_path", side_effect=[None, Path("/valid/path")])
@patch("wizard._detect_repos_in_dir", return_value=[])
@patch("wizard._prompt_yes_no", return_value=True)
def test_select_target_invalid_path_retry(
    mock_yes_no, mock_detect, mock_validate, mock_text, mock_choice
):
    """Test target selection with invalid path retry."""
    from wizard import select_target

    mode, path, _, _ = select_target()
    assert mode == "repos-dir"
    assert mock_validate.call_count == 2  # First invalid, second valid
...
@patch("wizard._validate_path", return_value=Path("/valid/path"))
@patch("wizard._detect_repos_in_dir", return_value=[])
@patch("wizard._prompt_yes_no", return_value=False)
def test_select_target_repos_dir_no_repos_decline(
    mock_yes_no, mock_detect, mock_validate, mock_text, mock_choice
):
    """Test declining when no repos detected."""
    from wizard import select_target

    # Patch prompt_text to return different values on subsequent calls
    with patch(
        "wizard._prompt_text", side_effect=["/empty/path", "/valid/path"]
    ), patch(
        "wizard._validate_path",
        side_effect=[Path("/empty/path"), Path("/valid/path")],
    ), patch(
        "wizard._detect_repos_in_dir", side_effect=[[], [Path("/valid/path/repo1")]]
    ):
        mode, path, _, _ = select_target()
        assert mode == "repos-dir"
...
@patch("wizard._prompt_yes_no", return_value=True)
@patch("wizard._prompt_text", side_effect=["invalid", "30"])
@patch("wizard._prompt_choice", return_value="")
@patch("wizard._get_cpu_count", return_value=4)
...
@patch("wizard._prompt_yes_no", return_value=True)
@patch(
    "wizard._prompt_text", side_effect=["1000", "30"]
...
def test_review_and_confirm_decline(mock_yes_no):
    ...
    from wizard import review_and_confirm
...
def test_generate_command_targets_mode():
    ...
    config.profile = "balanced"
    config.use_docker = False
    config.target_mode = "targets"
    config.target_path = "/path/to/targets.txt"
    config.results_dir = "results"
...
def test_generate_command_docker_no_mount():
    ...
    config.profile = "balanced"
    config.use_docker = True
    config.target_mode = "targets"  # Not repo or repos-dir
    config.results_dir = "results"
...
def test_execute_scan_decline(mock_yes_no):
    ...
    from wizard import execute_scan
...
def test_run_wizard_keyboard_interrupt():
    ...
    with patch("wizard.select_profile", side_effect=KeyboardInterrupt()):
...
def test_run_wizard_exception():
    ...
    with patch("wizard.select_profile", side_effect=Exception("Test error")):
        exit_code = run_wizard(yes=False)
