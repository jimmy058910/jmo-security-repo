...
def test_schedule_manager_list_with_label_filtering(tmp_path):
    ...
    names = [s.name for s in production_schedules]
...
def test_scan_schedule_to_dict():
    ...
    assert data["repos_dir"] == "~/repos"
    assert data["backend"] == "github-actions"
    assert data["description"] == "Test schedule"
...
def test_schedule_manager_to_dict_from_dict_round_trip(tmp_path):
    ...
    assert restored.repos_dir == original.repos_dir
...
def test_schedule_manager_label_filtering_with_none_labels(tmp_path):
    ...
    schedule_with_labels = ScanSchedule(
        name="with-labels",
        cron="0 1 * * *",
        profile="fast",
        repos_dir="~/repos",
    ...
    schedule_without_labels = ScanSchedule(
        name="without-labels",
        cron="0 2 * * *",
        profile="balanced",
        repos_dir="~/repos",
    ...
    assert prod_schedules[0].name == "with-labels"
...
def test_schedule_manager_update_labels(tmp_path):
    ...
    # Update labels
    schedule.labels = {"env": "production", "team": "security"}
    updated = manager.update(schedule)

    assert updated.labels == {"env": "production", "team": "security"}

    # Verify persistence
    retrieved = manager.get("update-labels")
    assert retrieved.labels == {"env": "production", "team": "security"}
