...
def test_minimal_gitlab_ci_generation(tmp_path):
    ...
    job = workflow["security-scan"]
    assert job["timeout"] == "30m"  # the one default when no deadline is set
...
def test_gitlab_ci_timeout_formats(tmp_path):
    ...
    # No deadline: the one default (there is no profile to vary it by)
    metadata.name = "no-deadline"
...
def test_gitlab_ci_header_comments(tmp_path):
    ...
    # A header describing a scan profile would describe something v2.0.0 lacks.
    assert "# Profile:" not in workflow_yaml
