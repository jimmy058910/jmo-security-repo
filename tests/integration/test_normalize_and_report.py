...
def test_gather_results_merges_and_dedupes(tmp_path: Path):
    ...
    write(r1 / "gitleaks.json", json.dumps([{"RuleID": "k1", "File": "a.txt", "StartLine": 1}]))
    write(r1 / "trufflehog.json", json.dumps({"DetectorName": "AWS", "Verified": True, "Line": 2}))
    write(r2 / "semgrep.json", json.dumps({"results": [{"check_id": "rule.x", "path": "b.py", "start": {"line": 3}, "extra": {"message": "m", "severity": "ERROR"}}]}))
    write(r2 / "noseyparker.json", json.dumps({"matches": [{"signature": "slack", "path": "c.txt", "line_number": 4}]}))
