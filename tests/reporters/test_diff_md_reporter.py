...
def test_markdown_footer(tmp_path, sample_diff_result):
    """The footer reports the running version, not a frozen literal.

    It said "v1.0.0" for every release since v1.0.1, because the string was
    hardcoded. Asserting the resolved version rather than a literal means the
    test cannot go stale the way the code did.
    """
    out_path = tmp_path / "diff.md"
    write_markdown_diff(sample_diff_result, out_path)
    ...
    assert f"**Generated by JMo Security v{get_jmo_version()}**" in content
