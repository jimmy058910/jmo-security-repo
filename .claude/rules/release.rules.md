...
| Bug archeology via `--maxfail=5` truncation | When the Nightly Extended Tests step runs `pytest tests/ ... --maxfail=5`, only the first 5 failures appear in output. Once you fix some of those 5, the next pass surfaces 5 *new* failures from deeper in the alphabetical test ordering — failures that have been latent for releases but were always invisible because earlier failures cut them off. The post-v1.0.3 stabilization went through 5 such layers (PRs #343 → #344 → #345 → #346 → #347), each fixing 3-5 visible failures and revealing 0-4 new ones. Approach: dispatch nightly via `gh workflow run scheduled.yml --ref main -f task=nightly` after each fix to trigger the next layer (~12-15 min/iteration vs 24h for cron) instead of waiting for 2AM UTC. |
