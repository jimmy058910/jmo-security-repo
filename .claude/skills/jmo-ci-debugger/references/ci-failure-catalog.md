...
```text
Error: uv.lock needs to be updated
Error: uv.lock is out of date with pyproject.toml (run: make deps-lock)
```

**Root Cause:**

`pyproject.toml` dependencies were edited but `uv.lock` was not regenerated.
...
# This runs: uv lock (regenerates uv.lock from pyproject.toml)
...
git add uv.lock
git commit -m "fix(ci): regenerate uv.lock"

# 3. Push to fix branch
git push origin fix/ci-broken

# 4. Create PR
gh pr create --title "Fix CI: stale uv.lock" \
  --body "Fixes recurring uv lock --check freshness check failure."
...
echo "=== Deps-compile ==="
grep "uv-lock\|uv lock" failures.txt
```

**Step 2: Fix in Order (Dependencies Matter)**

Fix in this order because a stale lock blocks the installs other tools need:

1. **uv-lock first** (a stale lock fails `uv sync --locked` for every later step):
...
pre-commit run markdownlint --all-files
pre-commit run mypy --all-files
make deps-lock && git diff uv.lock  # Should be clean
```
...
## 18. Bare `pip install` Lands Outside the uv Venv (post-uv.lock-migration)
...
============================== 1 skipped in 0.62s ==============================
make: *** [Makefile:158: test-e2e-visual] Error 5
```

Job dies in well under a minute. The install step **succeeded** — its log even says
`Installing collected packages: ... playwright, pytest-playwright`.

**Root Cause — a split interpreter:**

Since the uv.lock migration (PR #683, 2026-07-29), every job creates a real `.venv`,
and `Makefile:6` prefers it:

```make
PY := $(shell [ -x .venv/bin/python ] && echo .venv/bin/python || echo python3)
```

Pre-migration no `.venv` existed, so this fell through to system `python3` — the same
interpreter a bare `pip install` targets, which is why the pattern used to work.

**uv-created venvs ship no pip.** So even with `.venv/bin` first on PATH, a bare
`pip` resolves to the runner's *system* pip and installs there. `make` then runs
`.venv`'s pytest, which cannot import the package. A module-level
`pytest.importorskip("playwright")` skips the only module in the invocation, nothing
is collected, and **pytest exits 5**.

**The discriminator — do NOT audit every `pip install`:**

| Installed thing | Broken? | Why |
|---|---|---|
| A package pytest must **import** (`pytest-playwright`, any plugin/library) | **YES** | Import resolution is per-interpreter |
| A standalone **CLI invoked as its own binary** (semgrep, bandit, checkov, yamllint, pip-audit, build/twine) | No | Another interpreter's `bin/` is still on PATH |

A sweep of all four workflows found `e2e-visual` was the only importable-package site.
...
- name: Install Playwright
  run: |
    uv sync --locked --group dev --extra visual
    uv run --no-sync playwright install chromium --with-deps
```

`pytest-playwright` was already pinned in `uv.lock` under the `visual` extra, so this
is zero-drift. `--no-sync` keeps the browser-install step from touching the env at all.

**Measured nuance (uv 0.11.19) — do not repeat this error:** a plain `uv run` does
**not** strip the extra. Its *implicit* sync is **inexact** (top-up only), unlike
`uv sync`, which **is** exact-by-default. The two commands do not share sync
semantics. Keep `--no-sync` as insurance and to skip a pointless re-resolve, but do
not justify it with "otherwise the extra is stripped" — that was asserted, tested,
and found false.

**Verification — PR CI structurally cannot catch this:**

`scheduled.yml` jobs never run on `pull_request`, so a fully green PR proves nothing
about them. Dispatch the real suite on the branch instead:

```bash
gh workflow run scheduled.yml --ref <branch> -f task=e2e
```

Failing baseline `collected 0 items / 1 skipped` (exit 5) → after fix
`collected 12 items ... 12 passed in 4.29s`.

**Diagnostic shortcut:** pytest **exit code 5** means *nothing was collected*. In this
repo that almost always means a module-top `importorskip` lost its import — check
*which interpreter* installed the dependency before you look at the test.

**Time Investment:** 20-30 minutes (most of it confirming the interpreter split)
