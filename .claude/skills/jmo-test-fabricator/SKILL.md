...
**General:** coverage below the project's 85% target (CI itself fails only below 80%, `coverage-aggregate`'s "Verify coverage threshold" step), new CommonFinding schema features, improving coverage for any module.
...
- [ ] Coverage >=85% **for the new adapter module** (a local bar for new code, not
      a CI gate — CI's only floor is 85% repo-wide, `coverage-aggregate`'s "Verify coverage threshold" step, see #756):
