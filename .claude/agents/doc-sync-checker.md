...
**Output Format:** for each doc that needs updating, report the path, the
reason it is now stale, and the specific edit. Order by whether a user hits
the staleness (README, QUICKSTART, USER_GUIDE first). Do not estimate
completion time.
