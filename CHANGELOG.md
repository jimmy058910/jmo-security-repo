...
- **kubescape produces findings again — it had two independent reasons not to, and both are fixed.** It contributed **zero to every scan** on every Kubernetes repository, exiting 1 and writing no usable output. Measured end to end through `jmo scan`, on a three-manifest repository: **0 findings before, 51 after** (18 HIGH, 18 MEDIUM, 15 LOW).

  **The rules changed under a pinned binary.** `versions.yaml` pinned 4.0.12 throughout, and `tests/fixtures/golden/kubescape/v4.0.12/` holds 141,803 bytes of real output from that same version captured 2026-09-01, which still parses to 25 findings today. What changed is fetched at scan time: kubescape downloads its policy bundle into `~/.kubescape/` on every run, and the bundle now served carries controls whose rego the pinned evaluator cannot run — `no ValidatingAdmissionPolicy for control "C-0207" in embedded bundle`, and the same for `C-0275`, `C-0276` and `C-0295`, **four controls, not one**. Not a corrupt cache: deleting `allcontrols.json` re-downloads it and fails identically. The pin is now **4.0.13**, which kubescape's own startup warning names. ([#1211](https://github.com/jimmy058910/jmo-security-repo/issues/1211))

  **And `jmo.yml` passed it a flag it does not have.** `--silent` was set for `slim`, `balanced` and `deep` — every profile kubescape is in, including the default — and `kubescape scan --help` matches it **0 times**. An unknown flag makes it exit 1 before scanning anything, so JMo reported `findings are MISSING`, which reads exactly like a tool that ran and found nothing. This dates to 2025-12-14 and is independent of the version: with the same 4.0.13 binary and the same command, `kubescape.json` is **not written** with the flag and **179,810 bytes** without it. ([#1223](https://github.com/jimmy058910/jmo-security-repo/issues/1223))

  Fixing the version alone was not enough, and the entry that shipped with the version fix said otherwise — it was verified against the binary standalone, which is the one path that does not apply profile flags. **The transferable half is that a pinned version is not pinned behaviour, and a working binary is not a working invocation.** A scanner that fetches its rules at runtime can change what it does with nothing in this repository changing, and a config flag the tool rejects is untested by construction: the suite mocks `subprocess`, so it can assert the argv JMo builds and nothing about whether the binary parses it.

  This is the **third** instance of a shipped flag the tool rejects — after `trivy-rbac`'s `--scanners` and `prowler`'s `--quiet`, whose fix left a note three lines above the first kubescape `--silent` in the same file. A guard now names all three.
- **trivy-rbac now contributes findings. It never has.** Three independent defects sat in one tool's path, and each one alone was enough to reduce it to zero on every Kubernetes repository.

  It passed `--scanners config` to `trivy config`, which has no such flag at the pinned 0.74.0. The real binary answers `FATAL Fatal error unknown flag: --scanners`, exits 1 and writes no output file — so the tool could not start, in any profile, on any repository. The flag was redundant even where it might have parsed: `trivy config` *is* the misconfiguration scanner. (The flag that does exist on that subcommand is `--misconfig-scanners`, which selects config *formats* — terraform, kubernetes, helm — not scanner classes.) ([#1206](https://github.com/jimmy058910/jmo-security-repo/issues/1206))

  Whether it started at all was decided by three filename globs — `**/*deployment*.yaml`, `**/*service*.yaml`, `**/k8s/**/*.yaml` — which match **1 of 5 real manifest filenames**: `deployment.yaml` hits, and `deployment.yml`, `pod.yaml`, `ingress.yaml` and `configmap.yaml` all miss. `.yml` was not globbed once, in any of the three patterns, and `Pod`, `Ingress`, `ConfigMap`, `Role` and `NetworkPolicy` are all objects `trivy config` has checks for and none of them is called "deployment" or "service". The failure was silent and in the dangerous direction: `nothing for it to scan`, recorded on a repository with plenty. It now shares kubescape's content predicate — `apiVersion:` in a `.yaml` or `.yml`, at any depth, in either spelling. ([#1212](https://github.com/jimmy058910/jmo-security-repo/issues/1212))

  And underneath both, the adapter parsed a document trivy has never written. It read a top-level `checks` array of `{"checkID", "success"}`; `trivy config` writes `Results[].Misconfigurations[]` with `ID`, `Status` and `CauseMetadata`. So `data.get("checks", [])` returned `[]` and **56 real misconfigurations produced 0 findings**. All 14 of its unit tests passed, because all 14 hand-built the imagined shape. ([#1215](https://github.com/jimmy058910/jmo-security-repo/issues/1215))

  Measured end to end against trivy 0.74.0 on five manifests — the scanner builds the command, the real binary runs it, the real adapter parses the result: **exit 1 and no output file before; exit 0, 170,829 bytes and 56 findings after** (10 HIGH, 12 MEDIUM, 34 LOW), across all five files including the two the globs missed. Findings now carry the **path trivy scanned** and a **real line number** from `CauseMetadata.StartLine`; the old code synthesised `"<Kind>/<name>"` and line 0, a location matching no file, so nothing could group findings by file and deduplication had no real key to cluster on.

  **Fixing only the first two would have been worse than fixing none.** trivy-rbac would have exited 0, written a valid report and contributed nothing — with no error to read, because the no-output check that was reporting it would have stopped firing. Loudly broken is recoverable; silently broken is not. The guard against a repeat is a golden fixture of real captured output under `tests/fixtures/golden/trivy_rbac/`, which the adapter scored **0 of 19** on before this change.

- **A JMo results directory inside the scanned tree is no longer scanned.** `jmo scan . --out ./results` is the ordinary layout, and it puts JMo's output where the next scan walks it — so every tool reads JMo's own artifacts back as findings. They are not wrong exactly (a `syft.json` really does contain a "secret"), but they describe JMo's output rather than your code, and they grow every time you scan. Measured on OWASP Juice Shop during the v1.1.0 dogfood: **90 of 831 findings (10.8%)**. Measured on a two-file repository scanned twice: **11 findings, 9 of them JMo's own output — now 2, which are the real ones**.

  The results directory is now passed to every scanner's own exclusion mechanism, and the four spellings are not interchangeable — each was measured against the pinned binary rather than assumed. **horusec is included**, which matters because it was 88 of those 90 findings; it takes `-i` with glob paths, and a bare directory name excludes nothing there. Nothing is excluded when `--results-dir` points outside the repository, which is the usual CI setup.

  **This can over-exclude.** The exclusion is by directory *name*, because that is the only spelling all six tool grammars share — so a repository whose own source lives in a second directory of the same name loses it too. It only applies when JMo's output really is inside the tree being scanned. JMo's own file enumeration is exact and skips the results directory by path, so a same-named directory elsewhere is still scanned. ([#1156](https://github.com/jimmy058910/jmo-security-repo/issues/1156))

- **gosec and kubescape no longer report an ERROR on every repository that has no Go and no Kubernetes.** Both exit in about a tenth of a second writing no output file when there is nothing for them to load, and JMo reported each as `exited with an accepted code but wrote no output file - its findings are MISSING from this scan`. That is the message that catches a genuinely broken scanner — a Windows `.exe` omission once made trufflehog scan nothing, exit 0, and pass the `zero-secrets` policy — and firing it on every Node, Python, Java, Ruby and PHP repository trains you to ignore it. Measured on OWASP Juice Shop with both tools requested: **exit 1 and four ERROR lines before, exit 0 and none after**. They are now skipped when the repository has no `.go` file and no `go.mod`, and no `.yaml`/`.yml` containing `apiVersion:` respectively, and reported once at INFO as `SKIPPED with nothing to scan`.

  Kubernetes detection reads file **content**, not filenames: Juice Shop has 90 YAML files and none is a manifest, while `pod.yaml`, `ingress.yaml` and `configmap.yaml` are manifests that no name pattern catches. Vendored trees (`node_modules`, `.venv`, `vendor`, …) do not count toward either test, so a `.go` file shipped inside an installed package cannot trigger gosec on a repository that has no Go of its own.

  **The ERROR is unchanged where it is true.** A repository that does have Go or manifests still runs both tools, and a tool that runs and produces nothing is still reported as missing findings — "had nothing to scan" and "ran and failed" are now different outcomes with different messages. The same distinction now applies to the end-of-scan summary, which described both in the words of the first: only a tool that is not installed is reported as `STUBBED, not executed`. ([#1081](https://github.com/jimmy058910/jmo-security-repo/issues/1081))

- **`jmo tools uninstall` no longer appears to hang.** It listed `~/.jmo` by calling an `rglob("*")` plus a `stat()` per file on every subdirectory, before printing anything or asking anything — and `--dry-run` did not escape it, because the listing runs first. On a machine that has run `jmo tools install`, `~/.jmo/tools` holds the isolated virtualenvs: measured here at **165,419 files**, and the command sat silent for **89 seconds** to produce two size numbers. It now takes **0.5 s**. Directories are listed by name; a plain file still shows its exact size, which is one `stat()`. A capped walk was rejected — a few thousand files out of 165,419 gives a lower bound two orders of magnitude under the truth, and a wrong number in front of an irreversible delete is worse than no number. ([#1207](https://github.com/jimmy058910/jmo-security-repo/issues/1207))
...
The remaining user-reachable defects that ship open are listed in `docs/KNOWN_LIMITATIONS.md`, regenerated from the tracker rather than copied.

### Security

- **Fixed a stored XSS in `dashboard.html`.** Finding text taken from a scanned repository could break out of the dashboard's embedded-data `<script>` block and execute as HTML: the escaper handled a lowercase `</script>` while the HTML parser also ends a script element on `</SCRIPT>`, `</ScRiPt>` and `</script >`. Anyone who scans untrusted code and opens `dashboard.html` should upgrade. The same defect is fixed in `jmo diff --format html`. ([#865](https://github.com/jimmy058910/jmo-security-repo/pull/865))
- **`jmo ci --no-store-raw-findings` now actually stops raw findings being written to the history database**, and **`jmo ci --encrypt-findings` now actually encrypts.** Both flags were parsed, advertised in `--help`, and discarded, so a `jmo ci` run over a repository holding secrets persisted the matched source text to `.jmo/history.db` in plaintext, with exit code 0. `jmo scan` was never affected. Anyone who relied on either flag under `jmo ci` should treat their history database as containing raw finding data. ([#872](https://github.com/jimmy058910/jmo-security-repo/pull/872))
- **`jmo history query` now opens the database read-only.** It ran arbitrary SQL on a read-write connection, so `jmo history query "DROP TABLE findings"` really dropped the table while reporting an unrelated error. Write statements are now refused. ([#896](https://github.com/jimmy058910/jmo-security-repo/pull/896))
- **The `zero-secrets` policy now blocks verified secrets.** It gated on a field no scanner or adapter has ever produced, so `jmo policy test zero-secrets`, `jmo report --policy zero-secrets` and `jmo ci --fail-on-policy-violation` all reported PASS on scans containing verified credentials. `hipaa-compliance`'s allow condition was unconditionally true and now evaluates its rules. ([#924](https://github.com/jimmy058910/jmo-security-repo/pull/924))
- **`jmo verify` now checks the signature bundle** and reports `NOT CHECKED` rather than passing silently when no expected signer is named; it no longer reports success when no digest could be compared or tamper detection could not run. It gains `--signature`, `--cert-identity` and `--cert-oidc-issuer`, and `--rekor-check` now checks Rekor. An attestation whose sha256 is written in uppercase now verifies instead of reading as `TAMPER DETECTED`. ([#949](https://github.com/jimmy058910/jmo-security-repo/pull/949), [#950](https://github.com/jimmy058910/jmo-security-repo/issues/950))
- **The MCP server no longer logs `Authentication: enabled` while enforcing nothing**, and `mark_resolved` / `apply_fix` no longer report success for work they do not do. Callers are not authenticated on stdio; `get_server_info()` now says so in `authentication_enforced`. ([#955](https://github.com/jimmy058910/jmo-security-repo/pull/955))
- **Suppression expiries were written in UTC and evaluated against the local date**, so a suppression's effective lifetime was off by up to a day in either direction. Both sides now use UTC. ([#967](https://github.com/jimmy058910/jmo-security-repo/issues/967))
- **The trufflehog secret scan in the Docker validation no longer fails open**: a crashed scanner had reported success, and the CI test that exercised it ran nowhere. ([#965](https://github.com/jimmy058910/jmo-security-repo/pull/965), [#1029](https://github.com/jimmy058910/jmo-security-repo/pull/1029))
- `cryptography` stays at `>=50.0.0` for PYSEC-2026-3552, and tool installs can no longer downgrade it (see the isolated-install fix under *Scanning and tools*).

### Fixed

**Scanning and tools**

- **The kubescape adapter has never parsed real kubescape output.** It read a top-level key no release emits (both v3 and v4 carry `resourceIDs` plus an integer counter, with verdicts in `results[]`), so every Kubernetes scan silently yielded zero findings while the tool exited 0 and wrote a populated file. Measured on one insecure Pod manifest: 25 failing controls, 0 findings parsed, at both versions. Fixed and proven against real output from kubescape 3.0.47 and 4.0.12. ([#1094](https://github.com/jimmy058910/jmo-security-repo/issues/1094))
- **`jmo tools install <names>` and `jmo tools update` bypassed the isolation list.** prowler, semgrep and checkov are meant to live in their own environments because their pydantic pins conflict; only the profile installers honoured that, and every other install path ran pip against the interpreter itself. For a `pip install jmo-security` user, installing semgrep pinned their `mcp` to 1.29 and installing prowler downgraded `cryptography` below the CVE floor. All paths now isolate. ([#1101](https://github.com/jimmy058910/jmo-security-repo/issues/1101))
- **`jmo tools update` reported "All N tool(s) updated successfully" at exit 0 for pip tools it did not update**, printing the old version as `[OK]` two lines after warning about the mismatch. A pinned install whose resolved binary still reports another version is now a failure naming both versions, and the scanner and `tools check` resolve the same binary. ([#1093](https://github.com/jimmy058910/jmo-security-repo/issues/1093))
- **A tool that could not be run is no longer reported as having succeeded.** Under `--allow-missing-tools` an empty stub from a scanner that never executed was indistinguishable from a clean result, including to policies. ([#825](https://github.com/jimmy058910/jmo-security-repo/issues/825))
- **A target that produced nothing no longer reports as a success**, two target types that never worked now do, and a scan that scanned nothing no longer exits 0. ([#810](https://github.com/jimmy058910/jmo-security-repo/pull/810), [#826](https://github.com/jimmy058910/jmo-security-repo/pull/826))
- **`scancode` now runs its license and copyright detectors.** It was invoked with none, so it spent up to twenty minutes producing a file that could not contain a finding, and was graded successful. On native Windows, where its own bootstrap fails and upstream deprecates the platform, `jmo tools install scancode` now says so and names the workarounds instead of attempting the install. ([#835](https://github.com/jimmy058910/jmo-security-repo/issues/835), [#1091](https://github.com/jimmy058910/jmo-security-repo/issues/1091))
- **`profiles.<name>.tools` in `jmo.yml` is honoured.** A custom profile asking for one tool ran the full built-in profile instead, with no warning. Per-profile `per_tool` merges per key instead of replacing a tool's whole entry, and `per_tool` flags can no longer override the flags JMo must own. ([#975](https://github.com/jimmy058910/jmo-security-repo/issues/975), [#791](https://github.com/jimmy058910/jmo-security-repo/issues/791), [#831](https://github.com/jimmy058910/jmo-security-repo/pull/831))
- **`~` is expanded in every path-valued scan target.** A cron-installed schedule using `~/repos` previously scanned a literal `~` directory and reported nothing. ([#926](https://github.com/jimmy058910/jmo-security-repo/issues/926))
- **`jmo tools check` with no arguments exits non-zero when tools are missing**, matching its `--profile` form; a gate written `jmo tools check || exit 1` used to pass with scanners absent. ([#788](https://github.com/jimmy058910/jmo-security-repo/issues/788))
- `--skip-tools` now says which tools it skipped in a normal run. ([#871](https://github.com/jimmy058910/jmo-security-repo/issues/871))
- Tool output is no longer decoded with the host's locale; on Windows a single unmappable byte killed the reader thread silently, so the caller saw an empty capture rather than an error. ([#963](https://github.com/jimmy058910/jmo-security-repo/issues/963), [#936](https://github.com/jimmy058910/jmo-security-repo/issues/936))
- A tool timeout is now structurally detectable rather than inferred from a missing file. ([#734](https://github.com/jimmy058910/jmo-security-repo/pull/734))
- Unrecognised top-level keys in `jmo.yml` now produce a warning naming them and the recognised set instead of being discarded in silence, which is how `exclude_paths` (the real key is `exclude`) survived in the docs. ([#859](https://github.com/jimmy058910/jmo-security-repo/issues/859))
- trufflehog no longer reports pytest function names as verified secrets. ([#746](https://github.com/jimmy058910/jmo-security-repo/pull/746))

**Findings and reports**

- **`location.path` is now one repo-relative POSIX spelling.** One scan of one directory previously produced four spellings of the same file, and the scanning machine's home directory shipped into `findings.sarif` and `dashboard.html`. Because the path feeds the finding id, two spellings were two ids, so cross-tool deduplication could not see them. ([#861](https://github.com/jimmy058910/jmo-security-repo/issues/861))
- **`findings.sarif` is now valid SARIF 2.1.0.** A real 242-finding scan produced 340 schema errors and now produces 0; anyone whose SARIF was rejected by the GitHub Security tab or an IDE plugin should retry. Its `$schema` no longer points at a dead host, prose remediation moved from `fixes` (a machine-applicable edit in SARIF) to `properties.remediation`, and `driver.version` reports the running version instead of `1.0.0`. ([#868](https://github.com/jimmy058910/jmo-security-repo/pull/868))
- **`jmo report` from a pip-installed JMo now produces the real interactive dashboard.** Every released install produced a 2,468-byte static fallback page, because the built bundle was gitignored and absent from the wheel. Every `dashboard.html` now records which template produced it, `jmo report` warns when it did not use the built dashboard, and the fallback page carries the same security headers as the real one. ([#862](https://github.com/jimmy058910/jmo-security-repo/issues/862), [#864](https://github.com/jimmy058910/jmo-security-repo/issues/864), [#1033](https://github.com/jimmy058910/jmo-security-repo/pull/1033))
- **CWEs the tools report are now mapped.** `cweTop25_2024` was populated on 0 findings; OWASP and MITRE coverage rose with it. semgrep-secrets findings carry a usable CWE id. ([#845](https://github.com/jimmy058910/jmo-security-repo/issues/845), [#853](https://github.com/jimmy058910/jmo-security-repo/pull/853))
- **`jmo report` now validates findings against the CommonFinding schema, which it never actually did**: the schema path was wrong. The published `common_finding.v1.json` declares `cvss` as an object, matching what every adapter has always emitted. ([#763](https://github.com/jimmy058910/jmo-security-repo/pull/763), [#868](https://github.com/jimmy058910/jmo-security-repo/pull/868))
- `jmo report` now says so when a tool's output cannot be parsed, when NDJSON lines are lost, when a tool's JSON parses into something no tool emits, when compliance, EPSS/KEV or SBOM enrichment fails, when `outputs:` names a format it does not recognise, and when a requested artifact could not be written. All of these were silent or DEBUG-only. ([#830](https://github.com/jimmy058910/jmo-security-repo/pull/830), [#840](https://github.com/jimmy058910/jmo-security-repo/pull/840), [#853](https://github.com/jimmy058910/jmo-security-repo/pull/853))
- Files a tool reported it could not analyse are named in `SUMMARY.md` and at WARNING; a finding discarded for having no id is counted and attributed to its tool. ([#837](https://github.com/jimmy058910/jmo-security-repo/issues/837), [#848](https://github.com/jimmy058910/jmo-security-repo/issues/848))
- `--log-level` and `--json-logs`/`--human-logs` now reach adapter warnings. ([#842](https://github.com/jimmy058910/jmo-security-repo/pull/842))
- `outputs: []` now writes no files; four artifacts were written outside any gate. ([#867](https://github.com/jimmy058910/jmo-security-repo/issues/867))
- EPSS enrichment no longer issues one HTTP request per finding for CVEs it has no score for; a repeat `jmo report` on the same findings makes no requests at all. ([#849](https://github.com/jimmy058910/jmo-security-repo/issues/849))

**Suppressions**

- **`jmo.suppress.yml` now supports the `path`, `ruleId`, `severity` and `line` selectors** the docs and the shipped config had always advertised and the engine had never implemented. Anyone whose suppression file uses those keys will see their finding count drop, because entries that silently did nothing now apply; JMo's own shipped config went from 0 usable rules to 11. A file that cannot be parsed no longer crashes `jmo report` and applies no suppressions; an entry JMo cannot use, or an `expires:` it cannot read, is reported at WARNING instead of meaning "never expires"; `SUPPRESSIONS.md` lists the findings each rule caught. ([#860](https://github.com/jimmy058910/jmo-security-repo/pull/860))

**History and trends**

- `jmo fast`, `jmo balanced` and `jmo full` now record scans in history like `jmo scan` and `jmo ci` do; `jmo history` and `jmo trends` were permanently empty for anyone who only used the shortcuts. `slim` scans were silently discarded. ([#870](https://github.com/jimmy058910/jmo-security-repo/issues/870), [#725](https://github.com/jimmy058910/jmo-security-repo/pull/725))
- `jmo history` records the branch and commit of the repository that was scanned, the JMo version that produced the scan (every scan previously recorded `1.0.0`), the tools that actually ran, and a real duration (NULL on every row ever recorded). ([#793](https://github.com/jimmy058910/jmo-security-repo/pull/793), [#904](https://github.com/jimmy058910/jmo-security-repo/pull/904), [#981](https://github.com/jimmy058910/jmo-security-repo/issues/981))
- A single malformed finding no longer discards an entire scan's history; reading history no longer rewrites the database; a failed history write is reported at ERROR and in the summary line, and `--fail-on-store-error` makes it fatal. `jmo history` read commands work against a read-only database file. ([#901](https://github.com/jimmy058910/jmo-security-repo/issues/901), [#894](https://github.com/jimmy058910/jmo-security-repo/issues/894), [#905](https://github.com/jimmy058910/jmo-security-repo/pull/905), [#801](https://github.com/jimmy058910/jmo-security-repo/issues/801))
- `jmo history list` and `jmo history query` render an aligned table again; `--json` emits `[]` on no match; `jmo history stats` discloses what its tables leave out. ([#1011](https://github.com/jimmy058910/jmo-security-repo/issues/1011))
- **`jmo trends` analyses every branch by default** and reports the real improvement figures; net change, resolved, introduced and the score change were rendered from keys the analyser does not emit, so every report read `+0 findings (+0.0%) - STABLE`. `--days N` returns every scan in the window (a hidden 1000-row cap made `--days 365` and `--days 3650` identical), `trends show` works for any scan, `trends developers` no longer crashes and honours `--branch`, and `--export-html` produces a file for the first time. ([#917](https://github.com/jimmy058910/jmo-security-repo/pull/917), [#920](https://github.com/jimmy058910/jmo-security-repo/pull/920))

**Comparing scans**

- **`jmo diff` no longer reports a finding as fixed when a second tool starts agreeing with it.** Cross-tool clustering rewrites a finding's id, and diff keyed on that id, so a finding that gained a corroborating tool between two scans appeared once as resolved and once as new; measured on a real pair of scans, an unchanged `shell=True` injection was reported as resolved. A severity change is no longer reported as `unchanged`, and adding compliance mappings no longer reports a finding as `worsened`. ([#9749b14d](https://github.com/jimmy058910/jmo-security-repo/commit/9749b14d))
- `jmo diff --scan A --scan B` works (it defaulted to a database JMo has never written); `--format json` emits nothing but JSON on stdout; Markdown captured from stdout keeps its characters on a non-UTF-8 Windows console; `--severity` and `--tool` report a value they cannot use; a corrupt `findings.json` is no longer read as an empty one; the diff artifacts report the running version.

**Policies, schedules, validation and build**

- `production-hardening` no longer counts one finding as several violations; violations are no longer dropped when a finding has no `remediation`; `jmo policy show|list` print a policy's real metadata (every list-valued field was truncated at its first element); a missing `opa` is reported with an install hint instead of a traceback. ([#924](https://github.com/jimmy058910/jmo-security-repo/pull/924))
- **A schedule's targets now reach the workflow it generates.** The documented convenience factory wrote target keys no generator read, so the exported workflow ran `jmo scan` with no target and reported success. GitLab CI export carries all six target types and no longer drops `--url` targets; generated workflows quote their arguments; `schedule update --cron` updates the next run time; `schedule create` rejects what `schedule install` would; `jmo schedule` stores schedules in the same `~/.jmo` every other command uses; one unrecognised key in `~/.jmo/schedules.json` no longer makes every schedule unreadable; named weekdays and months are accepted. ([#932](https://github.com/jimmy058910/jmo-security-repo/pull/932), [#928](https://github.com/jimmy058910/jmo-security-repo/issues/928), [#934](https://github.com/jimmy058910/jmo-security-repo/issues/934), [#927](https://github.com/jimmy058910/jmo-security-repo/issues/927))
- **`jmo validate` no longer reports GO over checks that never ran**, no longer reports NO-GO on a correctly built tree, honours `--category`, reports the tier that actually ran, names the file each failing check found the problem in, and covers all 20 subcommands. ([#937](https://github.com/jimmy058910/jmo-security-repo/pull/937), [#774](https://github.com/jimmy058910/jmo-security-repo/pull/774), [#777](https://github.com/jimmy058910/jmo-security-repo/pull/777))
- **`jmo build` works again.** It could not locate the repository root after the `Dockerfile.deep` rename, its pre-build version gate could not pass at all, `build validate` reported "Validation passed" on checks it did not complete, `--variant` was silently overwritten, and it targeted images the project never publishes. ([#935](https://github.com/jimmy058910/jmo-security-repo/issues/935), [#939](https://github.com/jimmy058910/jmo-security-repo/issues/939))
- **`jmo ci` exits non-zero when the scan phase fails**, honours `--skip-tools`, `--resume`, `--no-resume` and `--collect-metadata`, no longer writes every report twice (two rows in the history database per run), and warns when `--fail-on` is a value it does not recognise. The files `jmo wizard` generates now contain commands that run; regenerate any wizard output you still use. ([#872](https://github.com/jimmy058910/jmo-security-repo/pull/872))
- `jmo wizard` no longer hands users a Docker tag three releases old: the tag is derived from `__version__`. ([#1075](https://github.com/jimmy058910/jmo-security-repo/issues/1075))
- `jmo mcp-server` no longer reads stdin when a dependency is missing, which consumed the client's `initialize` handshake; `query_findings_db` accepts queries whose data mentions a forbidden SQL keyword; MCP rate limiting is per principal. ([#958](https://github.com/jimmy058910/jmo-security-repo/issues/958), [#953](https://github.com/jimmy058910/jmo-security-repo/issues/953), [#952](https://github.com/jimmy058910/jmo-security-repo/issues/952))
- First-run setup no longer overwrites `~/.jmo/config.yml` or tells users to run commands that do not exist. ([#931](https://github.com/jimmy058910/jmo-security-repo/issues/931), [#790](https://github.com/jimmy058910/jmo-security-repo/issues/790))
- Console output no longer decides encodability from the codec's name, so warning glyphs render on every Windows console instead of only cp1252. ([#820](https://github.com/jimmy058910/jmo-security-repo/pull/820))
- `update_versions.py`, the sanctioned way to bump a tool pin, no longer deletes every comment in `versions.yaml`, and its `--check-latest` survives a legacy console codec. ([#1052](https://github.com/jimmy058910/jmo-security-repo/issues/1052), [#1090](https://github.com/jimmy058910/jmo-security-repo/pull/1090))

### Changed

- **Cross-tool clustering no longer merges two findings from the same tool**, and no longer merges genuinely different controls that shared a line. Reports will contain *more* findings than before; on a real scan 228 became 242, with `CKV_K8S_16` among those restored. `confidence.tool_count` now counts tools, not findings. ([#853](https://github.com/jimmy058910/jmo-security-repo/pull/853), [#846](https://github.com/jimmy058910/jmo-security-repo/issues/846))
- **The `deep` profile now runs `shellcheck`**, making it a superset of every other profile: 29 tools, 25 Docker-ready in `:deep`/`:latest`. ([#795](https://github.com/jimmy058910/jmo-security-repo/issues/795))
- **Attestations are emitted as in-toto Statement v1** (was v0.1), matching the SLSA Provenance v1 predicate they carry, so `cosign` and `slsa-verifier` can read them; v0.1 documents still verify. They record the scanned tree's git commit as a ResourceDescriptor, name their real subject, record `finishedOn`, and omit unmeasured defaults. ([#946](https://github.com/jimmy058910/jmo-security-repo/issues/946), [#945](https://github.com/jimmy058910/jmo-security-repo/issues/945))
- **Exit codes are documented and consistent**: usage errors are 2 throughout, `attest`/`verify` exit 2 for a missing path or flag, and `jmo trends`/`jmo history trends` exit 0 when a query legitimately matches nothing. See "Exit Codes" in `docs/CLI_REFERENCE.md`.
- `jmo trends show` and `jmo trends compare` no longer accept `--branch`; `--scan-ids` can no longer be combined with `--branch`, `--days` or `--last`; `--last` and `--days` reject values below 1.
- The CSV `triaged` column now means "a suppression rule matches but has expired"; it was `NO` for every row ever produced.
- MCP `mark_resolved` with `resolution="fixed"` writes no suppression and returns `success: false`; a fix is confirmed by re-scanning.
- All 29 scanner pins are current, including two majors: trivy 0.70 -> 0.74 and kubescape 3 -> 4. ([#1095](https://github.com/jimmy058910/jmo-security-repo/pull/1095))

### Added

- `jmo trends analyze --export-csv`, `--export-prometheus`, `--export-grafana` and `--export-dashboard`: the exporters existed and were documented, and no flag reached them.
- `--fail-on-store-error` on scans; `--log-level` and `--human-logs` on `jmo diff` and every `history`, `trends` and `policy` subcommand; `compliance` and `suppressions` as `outputs` values, both on by default. ([#879](https://github.com/jimmy058910/jmo-security-repo/issues/879), [#867](https://github.com/jimmy058910/jmo-security-repo/issues/867))
- `jmo trends developers --repo` and `--team-file`; the team-aggregation output was previously unreachable. ([#974](https://github.com/jimmy058910/jmo-security-repo/issues/974))
- Per-tool scan timings are persisted in `scan-timings.json`, including a GitLab target abandoned before any tool ran. ([#729](https://github.com/jimmy058910/jmo-security-repo/pull/729), [#824](https://github.com/jimmy058910/jmo-security-repo/issues/824))
- MCP `mark_resolved` records a decision as a suppression entry in `jmo.suppress.yml`; entries always expire (`expires_days`, default 90, maximum 365). ([#957](https://github.com/jimmy058910/jmo-security-repo/pull/957))
- `jmo adapters validate` fails on a file that defines no adapter, and `jmo adapters` with no subcommand exits 2 with usage.

### Removed

- `jmo verify --policy`, which never did anything. ([#943](https://github.com/jimmy058910/jmo-security-repo/issues/943))
- The `SKIP_REACT_BUILD_CHECK` environment variable; nothing had read it since March 2026.

### Documentation

- The tool catalogue is stated once: 29 scanners, profile sizes 9 / 13 / 17 / 29, 27 adapters, derived from the registry and guarded in CI. README, QUICKSTART, ROADMAP, SECURITY.md, the FAQ, the docs site index and the Homebrew formula all said 28 somewhere, and `jmo wizard --help` disagreed with `jmo --help` inside one binary. ([#1003](https://github.com/jimmy058910/jmo-security-repo/issues/1003))
- The Docker Hub description is rewritten for this release and pulls from Docker Hub; the README links the documentation site at docs.jmotools.com; `docs/KNOWN_LIMITATIONS.md` lists what actually ships open; `docs/SCHEDULE_GUIDE.md` and `docs/CLI_REFERENCE.md` no longer teach shapes that produce empty workflows or commands that exit 2. ([#1102](https://github.com/jimmy058910/jmo-security-repo/issues/1102), [#1103](https://github.com/jimmy058910/jmo-security-repo/issues/1103), [#1104](https://github.com/jimmy058910/jmo-security-repo/issues/1104), [#1106](https://github.com/jimmy058910/jmo-security-repo/issues/1106))
...
- **`jmo tools check` now distinguishes manual-install tools from genuinely missing tools across all output paths**: 4 tools in `PROFILE_TOOLS["deep"]` (`afl++`, `mobsf`, `akto`, `falco`) are intentionally NOT in the auto-install set — they require manual installation due to platform/size constraints (documented in `docs/MANUAL_INSTALLATION.md`). Pre-fix, every `jmo tools check` invocation lumped these in the red "7 tool(s) missing" line, conflating "you forgot to install this" with "this is expected to be absent in containerized usage". Now:
  - **Bare `jmo tools check`** (no-args profile-summary path): `deep` row reads `21 installed / 3 missing + 4 manual` instead of the misleading `21 installed / 7 missing`. Lower profiles (fast/slim/balanced) unchanged — they have no manual-install tools.
  - **`jmo tools check --profile <name>`**: per-tool table renders `MANUAL` (cyan) as a distinct status from `MISSING` (red), with hint text pointing at `docs/MANUAL_INSTALLATION.md`. Summary lines split: `3 tool(s) missing` + `4 tool(s) require manual install`.
  - **`jmo tools check --json`**: per-tool dict gains a new `manual_install: bool` field for programmatic consumers. `get_profile_summary` JSON output gains `real_missing` and `manual_install_missing` keys (`missing` retained as the union for back-compat).
  - **Exit code semantics preserved** — rc=1 when any tool is unavailable for any reason, including manual_install, so existing CI gates that fail-on-missing keep their semantics. Consumers that want to differentiate can parse the JSON. (PR #332's test logic that already worked around rc=1 on `:deep` containers continues to work.)
  - **Architecture**: added `MANUAL = "manual"` to `ToolStatusType` enum + `manual_install: bool = False` field to `ToolStatus`, threaded through existing `_derive_status_type` / `STATUS_COLORS` / `status_text` rendering pipeline so all downstream consumers (wizard, profile summary, etc.) automatically pick up the new state.

### Fixed

- **`Cross-Platform Tests (windows-latest)` job: 2 perf-threshold flakes on GHA Windows runners**: `test_attestation_generation_under_500ms` (`tests/unit/test_attestation_cicd.py:590`) and `test_large_scan_storage_performance` (`tests/unit/test_history_db_performance.py:93`) had hardcoded performance thresholds (0.5s and 8s respectively) that were tuned for Linux CI runners and Windows dev machines. GitHub Actions Windows runners are 2-3× slower for filesystem I/O and SQLite operations than the assumed thresholds — observed 1.371s and 20.71s in `task=all` dispatch run `24978569253`. Mirrors the existing `@pytest.mark.skipif(sys.platform == "win32", ...)` pattern from sibling test `test_single_scan_insert_performance` in the same file. Tests still run on Linux/macOS CI to catch regressions; functional correctness on Windows is covered by other tests in the same files. Surfaced via post-v1.0.4 verification — pre-existing flakes that were drowning in the CI noise pre-PR #350.
...
- **Branch coverage in `coverage-aggregate` merge — completes the PR #357 scope-limit**: The recompute pass in `.github/workflows/ci.yml`'s coverage merge handled `line-rate`/`lines-valid`/`lines-covered` only; `branch-rate`/`branches-valid`/`branches-covered` root attrs and per-line `condition-coverage` stayed stale at shard-1's standalone values. Two-stage fix: (1) per-line union — when both shards have `<line branch="true">` for the same line, set-intersect their `missing-branches` (a branch is missing in the union ONLY if missing in BOTH shards), recompute `condition-coverage` from `total - len(union_missing)`; (2) root recompute — walk merged `<line branch="true">` elements, sum `covered`/`total` from `condition-coverage`, set the root `branch-*` attrs. The CI threshold check still uses `line-rate` only, so this is currently informational, but Codecov can now read accurate aggregate branch coverage from the locally-merged `coverage.xml` (it previously had to re-derive from `<line>` elements — that path still works but is now consistent with the root attrs). Verified with synthetic two-shard fixtures: `{100, 200} ∩ {200, 300} = {200}` → 2/3 union, aggregate `3/4 = 75.00%`. Helpers `parse_missing_branches`/`parse_condition_total`/`merge_branch_attrs` added to the heredoc; absent `missing-branches` attribute correctly treated as `set()` (matches coverage.py's emission convention — the attribute is omitted, not emptied, when fully covered).

### Removed

- **`:full` Docker tag alias removed** — Per v1.0.3 release notes the `:full` raw tag (alias for `:deep`) was scheduled for removal "one release after v1.0.2"; v1.0.4 kept it as a deprecation cushion. Removing in v1.0.5 along with two coordinated fixes: (1) the `docker-size-benchmark` job's loop iterated `for VARIANT in full balanced slim fast` and tried to pull `:1.0.4-full` — a versioned-with-variant tag that has never existed (only the bare `:full` raw tag did), so the loop was silently producing empty cells in the size table. Loop now iterates `deep balanced slim fast` to match the actual matrix. (2) The size-comparison heredoc still labeled the heavyweight row as `Full: ~1.97 GB` — corrected to `Deep`.

### Internal

- **`update_versions.py --sync` now also updates `.github/workflows/*.yml` `env:` blocks** — Workflow `env:` blocks (introduced by PR #358 to harden tool installs against `install.sh` GitHub-API "latest" lookup flakes) declare pinned versions like `TRIVY_VERSION: "0.70.0"`. Pre-fix, only Dockerfiles were synced from `versions.yaml` — workflows required manual edits, creating drift risk. The `sync_dockerfiles()` function now walks `.github/workflows/*.yml` after Dockerfiles using a separate YAML-format regex (`{TOOL}_VERSION:\s+"[0-9.]+"`) — kept distinct from the Dockerfile shell-form pattern (`{TOOL}_VERSION="[0-9.]+"`) to avoid risk of corrupting YAML structure. Both file families now move atomically when `versions.yaml` is bumped. Verified via `--sync --dry-run`: 4 Dockerfiles + 4 workflows reported as in-sync.
- **`nightly-cross-platform` job filter** — Added `-m "not slow"` to both `pytest tests/unit/` and `pytest tests/cli/` invocations, matching the convention from `.claude/rules/testing.cross-platform.rules.md` "Workflow Marker Filter Convention". Replaces tactical PR #362 `@pytest.mark.skipif(sys.platform == "win32", ...)` decorators on individual perf-tests with the broader marker-filter pattern — future slow-test landings won't need per-test Windows skipif decorators to avoid GHA Windows runner perf-threshold flakes.
- **Stale `0.4.0` example refreshed in `docs/RELEASE.md`** — Post-release verification step showed `pip install --upgrade jmo-security==0.4.0` (predates v1.0.0). Bumped to current released version with a "(substitute the version you just tagged)" hint to make staleness less of a footgun.
- **`--maxfail` bump 5 → 20 on big-suite Nightly jobs + Step-Summary failure rollup** — Post-v1.0.3 stabilization required 5 successive PRs (#343 → #347) to drill through `--maxfail=5` truncation on `nightly-extended-tests` (~7,971 tests collected, 0.06% truncation cap). Bumped `--maxfail` to 20 on the three large-suite invocations: `nightly-extended-tests` (~7,971), `nightly-cross-platform` unit (~2,658, runs 6× in OS×Python matrix), `nightly-cross-platform` cli (~1,733). Smoke (`--maxfail=5`, 28 tests = 18% cap, real signal) and `e2e-tool-integration` (`--maxfail=3`, 4 tests, near-noop) kept at current thresholds. Added `--json-report --json-report-file=pytest-report.json` to `nightly-extended-tests` plus a `Summarize pytest failures` step that renders all caught failures (capped at 50 entries to fit GHA's ~1MB Summary cap) with tracebacks into the run's `$GITHUB_STEP_SUMMARY` — so when truncation does happen, the bug archeology cycle is collapsed from "5 PRs to drill through" to "1 PR to read all caught failures at once". `pytest-report.json` is now also bundled into the `nightly-test-results-${{ github.run_id }}` artifact for fully post-hoc analysis. `.claude/rules/testing.rules.md` and `release.rules.md` updated to reflect the new threshold + mitigation.
...
- **CI coverage merge has been a complete no-op since pytest-split was introduced — root-cause fix**: `coverage-aggregate` job in `ci.yml` had two compounding bugs that together meant `coverage.xml` was always byte-identical to shard-1's XML and the threshold check was always reading shard-1's coverage in isolation:
  1. **`hasattr(elem, '__iter__')` returns False for ElementTree Elements**: `process_package` used `classes = pkg.find('classes') or pkg.findall('class')` followed by `if hasattr(classes, '__iter__'): for cls in classes:`. Our cobertura XMLs nest `<package><classes><class>` (the standard format), so `pkg.find('classes')` returned the `<classes>` Element. But `xml.etree.ElementTree.Element` uses the legacy `__getitem__` iteration protocol — it does NOT define `__iter__`. So `hasattr(..., '__iter__')` was False, the for-loop body never executed, and `merged_files` stayed empty for every package. Replaced with `pkg.findall('.//class')` which works regardless of nesting.
  2. **Root `<coverage>` element's `line-rate`/`lines-valid`/`lines-covered` attributes were never recomputed**: even if bug #1 hadn't been there, the merge would correctly OR `<line>` hit counts but the threshold check reads `root.get('line-rate')` from the unchanged root attributes (shard-1's). Added a recompute pass that walks merged `<line>` elements and resets the root attributes before `coverage.xml` is written.
  - **Empirical proof** from CI run `24970815502`: shard-1 attr line-rate `0.6873`, shards 2-4 line-rates `0.7170` / `0.7154` / `0.7028`. With both bugs fixed, true merged line-rate computed from `<line>` elements is `0.8777` (87.77%) — matching what `make test` (sequential, full suite) reports.
...
- **CI install hardening — extends PR #350's Dockerfile convention to ALL workflow installs**: Replaced 14 `curl <upstream>/install.sh | sh` and `wget .../releases/latest/download/...` invocations across `ci.yml`, `scheduled.yml`, and `release.yml` with version-pinned downloads using the `--fail --retry 3 --retry-delay 5 --retry-all-errors --connect-timeout 30 --max-time 600` flag set + integrity check. Tools affected: trivy (4 sites), trufflehog (8 sites), grype (1 site), syft (1 site), hadolint (3 sites). The `install.sh` scripts internally call GitHub's API to discover the "latest" tag — a `curl --fail` short-circuit there triggered `aquasecurity/trivy crit unable to find ''` on commit `62bc02b` (run `24959553419`, 14:55 UTC 2026-04-26), breaking main CI for a Dockerfile-only PR. `releases/latest/download/...` redirects similarly drift from `versions.yaml` and aren't reproducible. Versions now pinned via job-level `env:` blocks (e.g., `TRIVY_VERSION: "0.70.0"`) keeping `versions.yaml` as source-of-truth. Convention documented in `.claude/rules/release.rules.md` "CI install hardening" entry.
- **Pre-commit hook: `yaml-env-tilde`**: New `scripts/dev/check_yaml_env_tilde.py` walks `.github/workflows/*.yml` and `.github/actions/**/*.yml`, parses each as YAML, walks `env:` blocks at any nesting level, and fails on any value that starts with `~/`. GitHub Actions does NOT shell-expand `~` in `env:` values — the `~` is passed as a literal string. PR #331 fixed `PRE_COMMIT_HOME: ~/.cache/pre-commit` after the literal `$PWD/~/.cache/pre-commit/` directory broke yamllint AND silently disabled `actions/cache` for the workflow's lifetime. This hook prevents that regression class. Does NOT flag `path:` in `actions/cache` `with:` blocks (Node-side `~`-expansion works) or `~/` in `run:` bash blocks (shell expansion works) — only the literal-pass-through `env:` case. 10 unit tests cover positive + negative paths.
