...
- **kubescape produces findings again.** It contributed **zero to every scan**, exiting 1 and writing a 0-byte output file on every repository with Kubernetes manifests — and **the binary never changed**. `versions.yaml` pinned 4.0.12 throughout, and `tests/fixtures/golden/kubescape/v4.0.12/` holds 141,803 bytes of real output from that same version, captured 2026-09-01, which still parses to 25 findings today.
  ...
  **The transferable half is larger than kubescape: a pinned version is not pinned behaviour.** A scanner that fetches its rules at runtime can change what it does with nothing in this repository changing, and neither CI nor the golden fixtures can see it — `test_adapter_golden.py` never invokes a binary, so a fixture stays green while the tool that produced it has stopped working. kubescape ships a `--controls-version` flag that would pin the rules too; adopting it changes which findings JMo reports, so it is tracked separately. ([#1211](https://github.com/jimmy058910/jmo-security-repo/issues/1211))
