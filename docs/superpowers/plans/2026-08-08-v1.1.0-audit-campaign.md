...
**A terminal state per finding is only half the bar.** The four states above
govern what a *chunk* surfaces. They say nothing about an issue that predates
the campaign, or one a chunk split out and no later chunk claimed — and those
accumulate silently, because filing an issue feels like resolution and is not.
Measured at the close of chunk 9: **112 issues open, roughly 40 claimed by no
chunk.** Every one had been correctly filed. None was scheduled.

So the bar has a second half, discharged in **chunk 22**:

> **Every open issue carries a release disposition before the tag.** Not a fix —
> a decision, recorded on the issue:
>
> - `BLOCKS-v1.1.0` — must be resolved first; becomes work, in chunk 22 or its
>   own chunk
> - `SHIP-WITH-IT` — accepted for this release; if a user can hit it, it goes in
>   [docs/KNOWN_LIMITATIONS.md](../../KNOWN_LIMITATIONS.md)
> - `DEFER-POST-1.1.0` — real, not now, **say why**
> - `CLOSE` — already fixed, superseded, or no longer applicable
> - `BLOCKED-UNVERIFIABLE` — carried through unchanged
>
> **The acceptance is that the count of undecided issues reaches zero, not that
> the count of open issues does.** An issue deliberately deferred with a stated
> reason is discharged; an issue nobody looked at is not, however small it
> looks.

This is deliberately a *disposition* bar and not a *fix* bar. Requiring 40 fixes
before a release is how a campaign stops shipping; requiring 40 decisions is a
session's work and makes "campaign complete" mean what it says.
...
| 22 | Backlog reconciliation + release readiness | **must be last**; the tag waits on it |

23 slots. The first draft had 17; the honest consequence of full scope with no
deferrals is a campaign ~30% larger than it first appeared. Better priced now
than discovered at chunk 14.

**Chunk 22 was added at the close of chunk 9**, once it was measurable that the
campaign's bar and the issue tracker had drifted apart. At that point 112 issues
were open and roughly 40 were claimed by no chunk — so "campaign complete" could
have been declared truthfully while a large backlog shipped unlooked-at. The
release bar below now covers both, and chunk 22 is where the second half is
discharged.
