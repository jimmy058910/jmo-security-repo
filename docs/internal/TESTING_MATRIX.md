...
**Coverage:** measure it before quoting it — CI's only enforced minimum is **85%** (`coverage-aggregate`'s "Verify coverage threshold" step), and nothing sets `--cov-fail-under` (#756)
...
- **Total Tests:** 8,000+
- **Coverage:** measure it before quoting it — CI's only enforced minimum is **85%** (`coverage-aggregate`'s "Verify coverage threshold" step); `--cov-fail-under` is set nowhere (#756)
