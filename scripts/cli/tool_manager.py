...
def get_remediation_for_tool(
    ...
    Returns:
        ...
        - 'manual': manual instructions when no command applies
...
class ToolStatus:
    ...
    def expected_version_display(self) -> str:
        """`expected_version` as a human should read it: a dash when unknown."""
        return self.expected_version or "-"
...
class ToolStatusSummary:
    ...
    different numbers for the same set of tools.
    """

    total: int  # Tools checked
    ...
    missing_dependency: list[str]  # Installed, but a runtime dependency is absent
    ...
    def needs_attention_count(self) -> int:
        """Count of tools needing attention."""
        return (
...
class ToolManager:
    ...
    def check_tool(self, tool_name: str) -> ToolStatus:
        ...
        if installed:
            ...
            if version_error:
                ...
                # Every tool JMo installs reports a version, so coming back with
                # nothing means the probe did not get an answer out of the
                # binary -- it crashed, timed out, or printed something
                # unparseable -- and "we could not confirm this tool runs" is
                # not OK.
                #
                ...
                #
                # It used to fire only for ISOLATED_TOOLS, on the reasoning that
                # a pip-pinned venv always knows its version while a binary's
                # `--version` "can legitimately be unparseable". Measured before
                # widening it (#1164): every matrix tool reports a version, and
                # the one that did not (lynis) has left. So a binary tool with
                # no version -- cdxgen under load, in #1164's reproduction --
                # printed OK at exit 0 as well.
                execution_ready = False
                execution_warning = (
                    f"{tool_name} is installed but did not report a version; it "
                    "may not run. Try: "
                    f"jmo tools install {tool_name} --force"
                )
            else:
    ...
    def check_matrix(
        self, tools: Collection[str] | None = None
    ) -> dict[str, ToolStatus]:
        """
        Check each tool in `tools` (default: the whole TOOL_MATRIX).
        ...
        check` inside the Docker image can exit silently (zero stdout,
        exit 1) when any one tool triggers an unexpected raise — the
        --json output never gets emitted because json.dumps is never
        reached.

        Args:
            tools: Tool names to check; TOOL_MATRIX when None

        Returns:
            Dict mapping tool name to ToolStatus
        """
        names = TOOL_MATRIX if tools is None else tools
    ...
    def get_missing_tools(
        ...
        """
        statuses = self.check_matrix(tools)
    ...
    def get_critical_outdated(
        ...
        `check_all_tools()` and version-probes the entire registry, including
        `ruff` and `shfmt`, for a scan that requested one tool.
        """
        outdated = self.get_outdated_tools(restrict_to=restrict_to)
        return [s for s in outdated if s.is_critical]

    def get_matrix_summary(self, tools: Collection[str] | None = None) -> dict:
        """
        Get summary statistics for a set of tools (Fix 1.4).

        Args:
            tools: Tool names to summarise; TOOL_MATRIX when None

        Returns:
            Dict with total, installed, execution_ready, missing, not_ready,
            outdated and critical_outdated counts, `ready` (every tool
            execution-ready) and the execution warnings of installed tools that
            cannot run.
        """
        statuses = self.check_matrix(tools)
    ...
    def get_version_drift(self, tools: Collection[str]) -> list[dict]:
        """
    ...
    def get_tool_summary(
        ...
        """
        Get unified tool status summary for a set of tools.
        ...
        Args:
            tools: Tool names to summarise; TOOL_MATRIX when None. Every tool
                inspected costs a `--version` subprocess, so a caller that only
                runs some tools should pass just those.
    ...
    def _find_binary(self, binary_name: str) -> str | None:
        ...
        # This ensures the isolated tools (semgrep, checkov) use their
        # isolated venv executables instead of any system version
    ...
    def _get_clean_env(self) -> dict:
        ...
        with it. Measured: that is why a Java tool's version probe reported
    ...
    def _verify_execution(self, tool_name: str) -> tuple[bool, str | None, list[str]]:
        ...
        """
        required = TOOL_EXECUTION_COMMANDS.get(tool_name) or [tool_name]
        ...
        for cmd in required:
            ...
            if tool_exists(cmd, warn=False):
                continue
            # Then try _find_binary for special paths (zap, isolated venvs, etc.)
    ...
    def _check_preinstall_deps(self, tool_name: str) -> list[str]:
        ...
        (e.g., Java for zap) before attempting to install the tool.

        Args:
            tool_name: Name of the tool to check

        Returns:
            List of missing dependency names (e.g., ["java"])
        """
        from scripts.core.install_config import DEPENDENCY_INSTALL_COMMANDS
        ...
        for cmd in required:
            ...
            if cmd == tool_name:
                continue
            # A dependency is whatever the installer knows how to install
            if cmd in DEPENDENCY_INSTALL_COMMANDS:
                if not tool_exists(cmd, warn=False) and cmd not in missing:
                    missing.append(cmd)
...
def print_tool_status_table(
    ...
    # Sort tools: missing or unable to run first (actionable), then outdated,
    # then OK.
    def sort_key(item):
        name, status = item
        ...
        # reader can fix it (install Java, add yara rules).
    ...
    for name, status in sorted(statuses.items(), key=sort_key):
        ...
        if not status.installed:
            ...
            # computed this - `_verify_execution` checks Java for zap and rules
            # for yara - and this table threw the answer away, so a box with no
            # `java` printed
