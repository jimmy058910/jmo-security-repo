...
def validate_python_version(
    ...
    if major < min_major or (major == min_major and minor < min_minor):
        if auto_reexec:
            ...
            print(f"{YELLOW}[hint]{RESET} Use: python3.12 -m pip install uv")
            print(
                f"{YELLOW}[hint]{RESET} Then: python3.12 scripts/dev/update_dependencies.py --compile"
            )
            return False
    print(
        f"{GREEN}[ok]{RESET} Python {major}.{minor} (meets requirement ≥{min_major}.{min_minor})"
    )
    return True
...
def check_requirements_txt_python_version(req_file: Path) -> tuple[bool, str]:
    ...
    with open(req_file) as f:
        for line in f:
            # pip-compile header: "# This file is autogenerated by pip-compile with Python X.Y"
            if line.startswith(
            ...
            # uv header: "#    uv pip compile --universal --python-version X.Y ..."
            if "--python-version" in line:
                idx = line.index("--python-version")
                rest = line[idx + len("--python-version") :].strip()
                version_str = rest.split()[0] if rest else ""
                if version_str:
                    return True, version_str
...
def run_pip_compile(upgrade: bool = False, repo_root: Path | None = None) -> int:
    """Run uv pip compile to regenerate requirements-dev.txt."""
    ...
    if upgrade:
        cmd.append("--upgrade")
        print(
            f"{BLUE}[info]{RESET} Running uv pip compile with --upgrade (will update all dependencies)"
        )
    else:
        print(
            f"{BLUE}[info]{RESET} Running uv pip compile (will preserve existing versions if compatible)"
        )
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"{RED}[error]{RESET} uv pip compile failed")
            print(result.stderr)
            return result.returncode
    except FileNotFoundError:
        print(f"{RED}[error]{RESET} uv not installed")
        print(f"{YELLOW}[hint]{RESET} Install with: {sys.executable} -m pip install uv")
        return 1
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            for conflict in conflicts:
                ...
                if any(pkg in conflict_lower for pkg in ISOLATED_VENV_PACKAGES) or any(pkg in conflict_lower for pkg in DEV_ONLY_PACKAGES):
...
def check_outdated_packages() -> list[dict[str, str]]:
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if outdated:
                print(
                    f"{YELLOW}[warn]{RESET} {len(outdated)} outdated package(s) detected:"
                )
                for pkg in outdated:
                    print(
                        f"  {YELLOW}•{RESET} {pkg['name']}: {pkg['version']} → {pkg['latest_version']}"
                    )
            else:
    ...
    if args.validate:
        ...
        if found:
            if (
                ...
                print(
                    f"{GREEN}[ok]{RESET} requirements-dev.txt compiled with {version_str}"
                )
        else:
            print(
                f"{YELLOW}[warn]{RESET} Could not detect Python version in requirements-dev.txt"
            )
    ...
    if args.compile or args.upgrade:
        ...
        if req_backup.exists():
            ...
            if downgrades:
                ...
                print(
                    f"{YELLOW}[hint]{RESET} Check if requirements-dev.in has version constraints causing downgrades"
                )
                exit_code = 1
        ...
        if not conflicts_ok:
            ...
            print(
                f"{YELLOW}[hint]{RESET} Review requirements-dev.in for conflicting version constraints"
            )
            exit_code = 1
