...
def run_pip_compile(upgrade: bool = False, repo_root: Path | None = None) -> int:
    ...
    if upgrade:
        cmd.append("--upgrade")
        print(
            f"{BLUE}[info]{RESET} Running pip-compile with --upgrade (will update all dependencies)"
        )
    else:
        print(
            f"{BLUE}[info]{RESET} Running pip-compile (will preserve existing versions if compatible)"
        )
    ...
    try:
        ...
        print(
            f"{YELLOW}[hint]{RESET} Install with: {sys.executable} -m pip install pip-tools"
        )
        return 1
