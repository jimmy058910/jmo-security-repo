...
update_gitleaks(){
  ...
  FILE="gitleaks_${TAG#v}_linux_x64.tar.gz"; [ "$ARCH" = "aarch64" ] || [ "$ARCH" = "arm64" ] && FILE="gitleaks_${TAG#v}_linux_arm64.tar.gz"
  URL="https://github.com/gitleaks/gitleaks/releases/download/${TAG}/$FILE"
  curl -fsSL "$URL" -o /tmp/gitleaks.tgz || { warn "download failed"; return 0; }
...
update_trivy(){
  ...
  FILE="trivy_${TAG#v}_Linux-64bit.tar.gz"; [ "$ARCH" = "aarch64" ] || [ "$ARCH" = "arm64" ] && FILE="trivy_${TAG#v}_Linux-ARM64.tar.gz"
  URL="https://github.com/aquasecurity/trivy/releases/download/${TAG}/$FILE"
  curl -fsSL "$URL" -o /tmp/trivy.tgz || { warn "download failed"; return 0; }
...
case "${1:-all}" in
  ...
  trivy) update_trivy ;;
  all) update_gitleaks; update_trivy ;;
