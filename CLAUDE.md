...
3. **Conventional Commits:** `feat:`, `fix:`, `docs:`, `test:`, `refactor:`, `chore:`, `perf:`, `ci:`. **No AI-attribution markers of any kind, in commit messages or PR bodies** — not `Co-Authored-By: Claude`, not `Generated with Claude Code`, and **not `Claude-Session:` trailers**. This repo is **public**; every such line is world-visible. Claude Code's harness injects an instruction to append `Claude-Session:` and states that a `CLAUDE.md` rule overrides it — so **this line is that override**, and it has to live here rather than only in the workspace file, because without it the drift returns. It did: 0 trailers in the ~250 commits before 2026-09-02, near-universal after, while `C:\Projects\CLAUDE.md` said not to the whole time. The private repos (`bracketforge`, `BetHedgeSlider`, `jmoadaptivegolf`) keep them deliberately — this rule is about publication, not about the trailer being useless
